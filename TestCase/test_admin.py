...
def test_post_add_admin_user_03():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('add_admin_user')['success'],
            response_dicts['success'])
    with pytest.allure.step("添加用户后查询数据库,断言是否查询出数据，是否del——flag为:0"):
        user_add_info = mysql_opt.data_read(
            "SELECT user_id,del_flag,phone FROM sys_user WHERE username = 'test1';",
            "bicai_admin")
...
def test_post_add_admin_user_again_04():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('add_admin_user')['success1'],
            response_dicts['success'])
    with pytest.allure.step("断言code对比"):
        test.assert_text(
            exp_results('add_admin_user')['code'], response_dicts['code'])
    with pytest.allure.step("断言message对比"):
        test.assert_text(
            exp_results('add_admin_user')['message'],
...
def test_get_admin_user_page_by_username_05():
    ...
    with pytest.allure.step("根据新创建用户test1查询用户管理页面"):
        response_dicts = req.start_request(
            "get_admin_user_userPage_by_username", username="test1")
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('get_admin_user_userPage_by_username')['success'],
            response_dicts['success'])
    Consts.RESULT_LIST.append('True')
...
def test_put_admin_user_modify_phone_06():
    ...
    """
    with pytest.allure.step("根据新创建用户test1查询用户管理页面"):
        response_dicts = req.start_request("modify_user_phone",
                                           userId=user_id,
                                           phone='13922222222')
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('modify_user_phone')['success'],
            response_dicts['success'])
    with pytest.allure.step("修改手机号后查询数据库,断言是否修改成功"):
        modify_phone = mysql_opt.data_read(
            "SELECT phone FROM sys_user WHERE user_id ='%s';" % user_id,
            "bicai_admin")[0]
...
def test_put_admin_user_modify_userinfo_07():
    ...
    with pytest.allure.step("修改用户test1密码"):
        response_dicts = req.start_request("modify_user_userinfo",
                                           userId=user_id,
                                           newpassword1='999999',
                                           newpassword2='999999')
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('modify_user_userinfo')['success'],
            response_dicts['success'])
...
def test_get_role_list_08():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('get_role_roleList')['success'],
            response_dicts['success'])
    Consts.RESULT_LIST.append('True')
...
def test_get_admin_menu_all_tree_09():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('get_menu_allTree')['success'],
            response_dicts['success'])
    Consts.RESULT_LIST.append('True')
...
def test_post_add_menu_10():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('add_menu')['success'], response_dicts['success'])
    with pytest.allure.step("添加菜单后查询数据库,断言是否查询出数据，是否del——flag为:0"):
        menu_add_info = mysql_opt.data_read(
            "SELECT menu_id,del_flag,parent_id FROM sys_menu WHERE name = '自动化测试';",
            "bicai_admin")
...
def test_get_admin_menu_by_id_11():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('get_menu_by_menuId')['success'],
            response_dicts['success'])
    Consts.RESULT_LIST.append('True')
...
def test_put_modify_menu_12():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('modify_menu')['success'], response_dicts['success'])
    with pytest.allure.step("修改菜单后查询数据库,断言是否查询出数据"):
        menu_modify_name = mysql_opt.data_read(
            "SELECT name FROM sys_menu WHERE menu_id = '%s';" % menu_id,
            "bicai_admin")[0]
...
def test_delete_del_menu_13():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('del_menu')['success'], response_dicts['success'])
    with pytest.allure.step("修改菜单后查询数据库,断言del_flag是否正确"):
        menu_del_flag = mysql_opt.data_read(
            "SELECT del_flag FROM sys_menu WHERE menu_id = '%s';" % menu_id,
            "bicai_admin")[0]
...
def test_post_add_role_14():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('add_role')['success'], response_dicts['success'])
    with pytest.allure.step("添加菜单后查询数据库,断言是否查询出数据，是否del——flag为:0"):
        role_add_info = mysql_opt.data_read(
            "SELECT role_id,del_flag FROM sys_role WHERE role_name = '我的自动化测试';",
            "bicai_admin")
...
def test_get_role_role_page_15():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('get_role_rolePage')['success'],
            response_dicts['success'])
    Consts.RESULT_LIST.append('True')
...
def test_put_modify_role_16():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('modify_role')['success'], response_dicts['success'])
    with pytest.allure.step("修改角色后查询数据库,断言是否查询出数据，是否del——flag为:0"):
        role_modify_info = mysql_opt.data_read(
            "SELECT role_name,role_code,role_desc,del_flag FROM sys_role WHERE role_id = '% s';"
            % role_id, "bicai_admin")
    ...
    test.assert_text(
        exp_results('modify_role')['del_flag'], str(role_del_flag))
    Consts.RESULT_LIST.append('True')
...
def test_get_role_tree_17():
    ...
    with pytest.allure.step("根据角色获取权限"):
        response_dicts = req.get_request("get_role_tree",
                                         str(role_modify_code))
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('get_role_tree')['success'], response_dicts['success'])
    with pytest.allure.step("查询数据库sys_role_menu,断言是否查询对应role_id:" +
                            str(role_id) + "的权限数据数据"):
        menu_ids = mysql_opt.data_read(
...
def test_put_role_menu_18():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('put_role_menu')['success'], response_dicts['success'])
...
def test_delete_role_20():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('delete_role')['success'], response_dicts['success'])
    with pytest.allure.step("删除角色后查询数据库,断言是否查询出数据，是否del——flag为:1"):
        role_del_info = mysql_opt.data_read(
            "SELECT role_name,role_code,role_desc,del_flag FROM sys_role WHERE role_id = '% s';"
            % role_id, "bicai_admin")
        role_del_flag = role_del_info[3]  # 删除后del_flag
    test.assert_text(
        exp_results('delete_role')['del_flag'], str(role_del_flag))
    Consts.RESULT_LIST.append('True')
...
def test_get_dept_tree_21():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('get_dept_tree')['success'], response_dicts['success'])
    Consts.RESULT_LIST.append('True')
...
def test_post_dept_22():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('post_dept')['success'], response_dicts['success'])
    with pytest.allure.step("删除角色后查询数据库,断言是否查询出数据，是否del——flag为:0"):
        dept_info = mysql_opt.data_read(
            "SELECT dept_id,code,del_flag FROM sys_dept WHERE name = '自动化测试';",
            "bicai_admin")
        global dept_id  # 部门id
        dept_id = dept_info[0]
...
def test_put_dept_23():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('put_dept')['success'], response_dicts['success'])
        put_dept_info = mysql_opt.data_read(
            "SELECT name,code,del_flag FROM sys_dept WHERE dept_id = '%s';" %
            dept_id, "bicai_admin")
        ...
        test.assert_text(
            exp_results('put_dept')['del_flag'], put_dept_del_flag)
    Consts.RESULT_LIST.append('True')
...
def test_del_dept_24():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('del_dept')['success'], response_dicts['success'])
        del_dept_info = mysql_opt.data_read(
            "SELECT name,code,del_flag FROM sys_dept WHERE dept_id = '%s';" %
            dept_id, "bicai_admin")
        del_dept_del_flag = del_dept_info[2]
        test.assert_text(
            exp_results('del_dept')['del_flag'], del_dept_del_flag)
    Consts.RESULT_LIST.append('True')
...
def test_get_dict_page_25():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('get_dict_page')['success'], response_dicts['success'])
    Consts.RESULT_LIST.append('True')
...
def test_post_dict_26():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('post_dict')['success'], response_dicts['success'])
    with pytest.allure.step("添加字典后查询数据库,断言是否查询出数据"):
        dict_add_info = mysql_opt.data_read(
            "SELECT id,type,del_flag FROM sys_dict WHERE remarks = '自动化测试123';",
            "bicai_admin")
...
def test_put_dict_27():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('put_dict')['success'], response_dicts['success'])
    with pytest.allure.step("修改字典后查询数据库,断言是否查询出数据"):
        dict_put_info = mysql_opt.data_read(
            "SELECT type,del_flag,remarks FROM sys_dict WHERE id = '% s';" %
            dict_id, "bicai_admin")
        ...
        test.assert_text(
            exp_results('put_dict')['del_flag'], put_dict_del_flag)
...
def test_del_dict_28():
    ...
    with pytest.allure.step("删除字典"):
        response_dicts = req.del_request("del_dict", str(dict_id),
                                         put_dict_type)
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('del_dict')['success'], response_dicts['success'])
    with pytest.allure.step("删除字典后查询数据库,断言是否查询出数据"):
        dict_del_info = mysql_opt.data_read(
            "SELECT del_flag FROM sys_dict WHERE id = '% s';" % dict_id,
            "bicai_admin")
...
def test_get_admin_user_delete():
    ...
    with pytest.allure.step("断言success对比"):
        test.assert_text(
            exp_results('delete_user_by_userid')['success'],
            response_dicts['success'])
    with pytest.allure.step("添加用户后查询数据库,断言是否查询出数据，是否del——flag为:1"):
        delde_flag = mysql_opt.data_read(
            "SELECT del_flag FROM sys_user WHERE user_id ='%s';" % user_id,
            "bicai_admin")[0]
        del_data()  # 数据回收
...
def del_data():
    with pytest.allure.step("数据回收"):
        mysql_opt.data_write(
            "DELETE FROM sys_menu WHERE menu_id = '%s' ;" % menu_id,
            "bicai_admin")
        ...
        mysql_opt.data_write(
            "DELETE FROM sys_role_dept WHERE role_id = '%s' ;" % role_id,
            "bicai_admin")

        mysql_opt.data_write(
            "DELETE FROM sys_user_role WHERE role_id = '%s' ;" % role_id,
            "bicai_admin")

        mysql_opt.data_write(
            "DELETE FROM sys_role_menu WHERE role_id = '%s' ;" % role_id,
            "bicai_admin")

        mysql_opt.data_write(
            "DELETE FROM sys_dict WHERE id = '%s' ;" % dict_id, "bicai_admin")
